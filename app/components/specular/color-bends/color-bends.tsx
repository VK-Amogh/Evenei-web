...
import styles from './color-bends.module.css';
...
const ColorBends = forwardRef<HTMLDivElement, ColorBendsProps>(function ColorBends({
  className,
  style,
  rotation = 45,
  speed = 0.2,
  ...
  const colorVecs = useMemo(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      if (colors[i]) {
        ...
        arr.push(new THREE.Vector3(r, g, b));
      } else {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i++) {
      ...
      if (colorVecs[i]) vec.copy(colorVecs[i]);
      else vec.set(0, 0, 0);
    }
